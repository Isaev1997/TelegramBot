...
async Task HandleUpdateAsync(ITelegramBotClient bot, Update update, CancellationToken cancellationToken)
{
    ...
    else if (update.CallbackQuery != null)
    {
        ...
        if (data == "history")
        {
            ...
            await bot.SendTextMessageAsync(chatId, "🏛 История местности:", cancellationToken: cancellationToken);
            await bot.SendTextMessageAsync(
                chatId,
                $"🔗 [Открыть поиск в Википедии]({wikiSearchUrl})",
...
// ✅ ОБЯЗАТЕЛЬНО вне HandleUpdateAsync
